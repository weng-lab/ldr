...
from ..constants.constants import HUMAN_SOMATIC_CHROMOSOMES, BASELINE_FREQUENCIES_URL
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix = ".tar.gz") as tar:
                if os.system("wget {url} -O {tar}".format(url = BASELINE_FREQUENCIES_URL, tar = tar.name)) != 0:
                    raise ChildProcessError("failed to save %s to %s; check your network connetion and try again" % (BASELINE_FREQUENCIES_URL, tar))
